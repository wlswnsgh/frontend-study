...
const counterSlice = createSlice({
  ...
  reducers: { // reducer들을 정의
    increment: (state) => { // 첫번째 파라미터: 현재 state를 받아옴(실제로는 state의 복사본)
      ...
      // 어떻게? Immer 라이브러리 내장되어 불변성 관리를 해주기 때문에
      // 실제로는 상태를 직접 변경하지 않고 내부적으로 state 복사본을 만들어서 그 본사본을 변경하고 새로운 상태를 반환함
    },
    decrement: (state) => { // 첫번째 파라미터: 현재 state를 받아옴(실제로는 state의 복사본)
      state.value -= 1;
    },
...
// 2) 다른데서 사용할 수 있게 액션 생성 함수를 생성
// 3) 수정을 원할 때 그 함수를 실행해달라고 store에 요청을 해야함

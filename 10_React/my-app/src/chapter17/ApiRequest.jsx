...
function ApiRequest() {
  ...
  // promise/then
  const handleRequestById = (e) => {
    axios.get(`https://jsonplaceholder.typicode.com/photos/${e}`)
    .then((response) => {
  ...
  // Quiz
  // 2. async/await로 리팩터링
    const handleRequestByIdAsync = async (e) => {
    try {
      let res = await axios.get(`https://jsonplaceholder.typicode.com/photos/${e}`);
      setData(res.data);
    } catch (error) {
      console.error(error);
    }
